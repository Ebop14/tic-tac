...
# Standard 8x8 checkers geometry, but with one rule relaxed: captures are NOT
# forced. A player may always choose a quiet (non-capturing) move even when a
# jump is available. This makes for a looser, more forgiving game than the
# checkers8 variant, which mandates the capture.
#
# Dark (playable) squares are those where (r + c) is odd, numbered 1..32
...
class Board:
    ...
    def legal_moves(self):
        # Captures are optional in this variant: offer jumps AND quiet moves
        # together rather than forcing the jump when one exists. A multi-jump
        # chain, once begun, must still be played to completion (that logic
        # lives in _find_jumps), but starting it is the player's choice.
        return self._jump_moves() + self._simple_moves()
...
#
# As in checkers8, the "teacher" is alpha-beta to a fixed depth with a material
# + advancement evaluation at the leaves. Because captures are optional here,
# the search naturally weighs declining a jump against taking it, so the labels
# reflect genuinely optimal play under the relaxed rules. Scores are from
# BLACK's perspective: positive is good for BLACK, negative good for WHITE.
